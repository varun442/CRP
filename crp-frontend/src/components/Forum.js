...
const ForumFeed = () => {
    ...
    const [chats, setChats] = useState([
        { id: 1, username: 'JohnDoe', lastMessage: 'Hey, how are you?', unread: 2 },
        { id: 2, username: 'JaneSmith', lastMessage: 'See you at the event!', unread: 0 },
        { id: 3, username: 'BobJohnson', lastMessage: 'Thanks for your help!', unread: 1 },
        { id: 4, username: 'AliceWilliams', lastMessage: 'Did you see the new community post?', unread: 3 },
        { id: 5, username: 'CharlieGreen', lastMessage: 'I have a question about the upcoming meeting.', unread: 0 },
        { id: 6, username: 'EvaBlack', lastMessage: 'The park cleanup was a great success!', unread: 1 },
        { id: 7, username: 'DavidWhite', lastMessage: 'Can you help me with a neighborhood issue?', unread: 0 },
    ]);
    ...
    const handleCreatePost = () => {
        const post = {
            ...
            ...newPost,
            author: { _id: 'currentUserId', username: 'CurrentUser' },
            likes: [],
            comments: [],
    ...
    const handleChatSelect = (chatId) => {
        ...
        setChatMessages([
            { id: 1, sender: chats.find(c => c.id === chatId).username, content: 'Hey, how are you?', timestamp: '2023-09-22T10:30:00Z' },
            { id: 2, sender: 'CurrentUser', content: "I'm doing great, thanks! How about you?", timestamp: '2023-09-22T10:31:00Z' },
            { id: 3, sender: chats.find(c => c.id === chatId).username, content: "I'm good too. Are you coming to the community event this weekend?", timestamp: '2023-09-22T10:32:00Z' },
        ]);
    };
            ...
            <div className="container mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-8">
                        <div className="bg-white rounded-lg shadow-lg p-6">
                            ...
                            <div className="space-y-4 max-h-96 overflow-y-auto">
                                {chats.map((chat) => (
                                    ...
                                    >
                                        <img src={`https://api.dicebear.com/6.x/initials/svg?seed=${chat.username}`} alt={chat.username} className="w-10 h-10 rounded-full mr-3" />
                                        <div className="flex-grow">
                                            <h3 className="font-semibold">{chat.username}</h3>
                                            <p className="text-sm text-gray-500 truncate">{chat.lastMessage}</p>
                                        </div>
                                        {chat.unread > 0 && (
                                            <span className="bg-blue-500 text-white text-xs font-bold px-2 py-1 rounded-full">{chat.unread}</span>
                                        )}
                                    </div>
                                ))}
                            </div>
                        ...
                        {selectedChat && (
                            <div className="bg-white rounded-lg shadow-lg p-6">
                                <h2 className="text-xl font-semibold mb-4">Chat with {chats.find(chat => chat.id === selectedChat)?.username}</h2>
                                <div className="h-64 overflow-y-auto mb-4">
                                    {chatMessages.map((message) => (
                                        <div key={message.id} className={`mb-2 ${message.sender === 'CurrentUser' ? 'text-right' : 'text-left'}`}>
                                            <div className={`inline-block p-2 rounded-lg ${message.sender === 'CurrentUser' ? 'bg-blue-100' : 'bg-gray-100'}`}>
                                                <p>{message.content}</p>
            ...
            <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center">
                        <img src={`https://api.dicebear.com/6.x/initials/svg?seed=${post.author.username}`} alt={post.author.username} className="w-10 h-10 rounded-full mr-4" />
                        <div>
                ...
                <div className="flex items-center justify-between text-gray-500">
                    ...
                    <button onClick={() => setShowComments(!showComments)} className="flex items-center hover:text-blue-600 transition-all duration-300">
                    ...
                    {post.comments.map((comment) => (
                        <div key={comment._id} className="mb-4 last:mb-0">
                            <div className="flex items-start">
                                <img src={`https://api.dicebear.com/6.x/initials/svg?seed=${comment.author.username}`} alt={comment.author.username} className="w-8 h-8 rounded-full mr-3" />
                                <div className="flex-grow">
                                    ...
                                    <div className="flex items-center mt-2 text-sm text-gray-500">
                                        <button className="mr-4 hover:text-blue-600 transition-all duration-300">Like</button>
                                        <button className="mr-4 hover:text-blue-600 transition-all duration-300">Reply</button>
